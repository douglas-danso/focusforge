...
export default function TaskCreateModal({ onClose, onSuccess }: TaskCreateModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // Use AI-enhanced task creation from orchestrator
      const result = await orchestratorAPI.createEnhancedTask({
      ...
      addNotification({
        type: 'success',
        title: 'Task Created',
        message: 'Task created with AI-powered breakdown!'
      });
      onSuccess();
    } catch (error: any) {
      console.error('AI task creation failed, falling back to regular creation:', error);
      // Fallback to regular task creation if AI fails
      try {
        await createTask({
          title: formData.title,
          description: formData.description,
        ...
        addNotification({ type: 'success', title: 'Task Created', message: 'Task created successfully!' });
        onSuccess();
      } catch (fallbackError: any) {
        addNotification({ type: 'error', title: 'Creation Failed', message: fallbackError.message || 'Failed to create task' });
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleAIPreview = async () => {
    ...
    try {
      // Get AI analysis from orchestrator without creating the task yet
      const result = await orchestratorAPI.createEnhancedTask({
        title: formData.title,
        description: formData.description,
        duration_minutes: formData.duration_minutes,
        ...
        deadline: formData.deadline || undefined,
        tags: formData.tags,
        preview_only: true, // Request preview mode
      });

      if (result.data?.ai_analysis) {
        setAiBreakdown(result.data.ai_analysis);
      } else {
        ...
        setAiBreakdown({
          ...
          recommended_approach: 'Break this into focused sessions with regular breaks',
          suggested_ritual_duration: 5,
          breakdown: [
            { title: 'Planning & Setup', estimated_minutes: Math.max(5, Math.floor(formData.duration_minutes * 0.1)), energy_level: 'medium' },
            { title: 'Core Work', estimated_minutes: Math.floor(formData.duration_minutes * 0.8), energy_level: 'high' },
            { title: 'Review & Wrap-up', estimated_minutes: Math.max(5, Math.floor(formData.duration_minutes * 0.1)), energy_level: 'low' },
          ],
        });
      }
    } catch (error) {
      ...
      setAiBreakdown({
        ...
        estimated_blocks: Math.ceil(formData.duration_minutes / 25),
        procrastination_risk: 'medium',
        recommended_approach: 'Focus on one step at a time with regular breaks',
        breakdown: [
          { title: 'Start & Setup', estimated_minutes: 10, energy_level: 'medium' },
          { title: 'Main Work', estimated_minutes: formData.duration_minutes - 15, energy_level: 'high' },
          { title: 'Review', estimated_minutes: 5, energy_level: 'low' },
        ],
      });
    } finally {
      setIsLoading(false);
